...
bool checkVerticalWins(Board *b)
{
  ...
  for (int row = 0; row < 3; row++)
  {
    ...
    if (result)
    {
      ...
      winningRow = row;
#ifdef DEBUG
      printf("vertical win found!  test row = %i", winningRow);
#endif
    }
  }
  return winFound;
...
int eq(enum type t, int x, int y)
{
  ...
  {
    if (t == CHAR)
      fprintf(stderr, "Test %d gives %c not %c\n", n, x, y);
    if (t == INT)
      fprintf(stderr, "Test %d gives %d not %d\n", n, x, y);
    if (t == BOOL && x)
      fprintf(stderr, "Test %d gives true not false\n", n);
    if (t == BOOL && y)
      fprintf(stderr, "Test %d gives false not true\n", n);
    exit(1);
  }
  return n;
}
